...
def print_links(run_ids: list[str], by_run_id: dict[str, str]) -> list[str]:
    missing: list[str] = []
    print("")
...
def main(argv: list[str]) -> int:
    ...
    if not run_ids:
        print("Usage: print_run_frontend_links.py <run_id> [run_id ...]", file=sys.stderr)
        return 2
    ...
    missing = print_links(run_ids, by_run_id)
    if missing:
        print(
